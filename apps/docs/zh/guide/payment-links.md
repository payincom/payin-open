...
支付链接是 PayIn Cloud 的无代码功能，用于通过可分享链接接受加密货币支付。PayIn Open 不随附该无代码功能或仪表板应用；自托管部署应使用订单支付 API，或构建自己的控制台链接流程。

## 什么是支付链接？

支付链接是 PayIn Cloud 的无代码仪表板功能。PayIn Open 不随附无代码仪表板应用；在自托管部署中，请通过订单支付 API 或您自己的控制台构建等效支付流程。

**主要特性：**
- 🚫 **Cloud-only 无代码流程** - PayIn Open 不随附
- 🔗 **随处分享** - 电子邮件、WhatsApp、社交媒体、二维码
- 💰 **灵活定价** - 固定金额或自定义金额
- 🌐 **多链支持** - 支持多个网络和代币
- 📊 **追踪支付** - 在 PayIn Cloud 仪表板中监控所有支付
- 📦 **库存控制** - 限制门票/产品数量
...
| 功能 | 支付链接 | 订单支付 API |
|---------|---------------|-------------------|
| **设置** | PayIn Cloud 无代码流程 | 需要开发 |
| **分享** | 公开 URL/二维码 | 程序化集成 |
| **使用场景** | 手动分享、简单需求 | 自动化系统、电子商务 |
| **自定义** | 有限（通过界面） | 完全控制（通过代码） |
| **支付追踪** | PayIn Cloud 仪表板 | 自建控制台 + Webhook + API |
...
1. 登录 PayIn Cloud 仪表板（Cloud-only 功能）
2. 在侧边栏导航到 **支付链接**
3. 点击 **创建支付链接**
4. 填写详细信息：
   - **标题**: "咨询服务 - 1 小时"
...
- 📚 **PayIn Cloud 仪表板**: Cloud-only 支付链接的内置帮助和教程
