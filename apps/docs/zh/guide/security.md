...
- 📊 **完整审计日志**：所有操作都会记录并可追溯
- 🔑 **密钥范围控制**：按 Open 部署和业务/API-key 范围限制访问
...
PayIn Open 将 API 访问限定在您的自托管部署和业务/API-key 范围内。Open 不随附 Cloud 组织成员或仪表板角色；请使用身份提供商、部署平台控制或自建控制台保护 operator 功能。

| 密钥范围 | 权限 | 使用场景 |
|------|-------------|----------|
| **Operator** | 管理部署设置、密钥、webhook、地址池 | 可信运营自动化 |
| **写入** | 创建订单和充值，读取相关状态 | 应用后端 |
| **只读** | 查询状态和历史 | 监控、分析 |
