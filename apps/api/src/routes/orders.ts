 ...
 */
orders.get('/', authMiddleware(), requirePermission('orders:read'), async c => {
  try {
    const manager = getManager();
        ...
        return c.json(
          {
            ...
            suggestions: ['asc returns oldest records first', 'desc returns newest records first'],
          },
          400
        );
      }
 ...
 * Required permission: orders:read
 */
orders.get('/stats', authMiddleware(), requirePermission('orders:read'), async c => {
  try {
    const manager = getManager();
 ...
 * Required permission: orders:read
 */
orders.get('/:id', authMiddleware(), requirePermission('orders:read'), async c => {
  try {
    const manager = getManager();
