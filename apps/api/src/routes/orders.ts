...
export function createOrdersRoutes(deps: OrdersRouteDependencies = {}) {
          ...
          return c.json(
            {
              ...
              message:
                'Amount must be a numeric string (e.g., "10.50") with up to 18 decimal places',
              suggestions: [
   ...
   */
  orders.get('/', authMiddleware(), requirePermissionMiddleware('orders:read'), async c => {
    try {
      const manager = getManagerInstance();
      ...
      const result = await manager.listOrdersForRuntimeScope(runtimeContext, filters);

      // Add payment URLs to orders
      const baseUrl = getBaseUrlHelper();
      const enrichedOrders = result.orders.map((order: any) => ({
        ...order,
        url: buildOrderPaymentUrlHelper(baseUrl, order.id),
      }));
   ...
   * Required permission: orders:read
   */
  orders.get('/stats', authMiddleware(), requirePermissionMiddleware('orders:read'), async c => {
    try {
      const manager = getManagerInstance();
      ...
      const stats = await manager.getOrderStatisticsForRuntimeScope(runtimeContext, filters);
   ...
   * Required permission: orders:read
   */
  orders.get('/:id', authMiddleware(), requirePermissionMiddleware('orders:read'), async c => {
    try {
      const manager = getManagerInstance();
      ...
      const order = await manager.getOrderForRuntimeScope(orderId, runtimeContext);
