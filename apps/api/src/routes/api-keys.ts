...
export interface ApiKeysRouteDependencies {
...
export function createApiKeysRoutes(deps: ApiKeysRouteDependencies = {}) {
  ...
  apiKeys.post(
    '/',
    authMiddleware(),
    requirePermissionMiddleware('api-keys:write'),
    ...
    async c => {
      try {
        ...
        const result = await authManager.createApiKeyForRuntimeScope(userId, runtimeContext, {
          name: body.name,
          expiresAt,
        });
   ...
   */
  apiKeys.get('/', authMiddleware(), requirePermissionMiddleware('api-keys:read'), async c => {
    try {
      const authManager = getAuthManager();
      const userId = c.get('userId') as string;
      const runtimeContext = resolveRuntimeContext(c);
      ...
      const keys = await authManager.listApiKeysForRuntimeScope(runtimeContext, userId);
   ...
   * Required permission: auth:read
   */
  apiKeys.get('/:id', authMiddleware(), requirePermissionMiddleware('api-keys:read'), async c => {
    try {
      const authManager = getAuthManager();
      const userId = c.get('userId') as string;
