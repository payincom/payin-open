...
export class ConfigValidator {
  ...
  private readonly knownChains = new Set([
  ...
  validateConfig(config: LoadedConfig, requestedChains?: string[]): ValidationResult {
    // Get available providers from config + built-in provider templates
    const configProviders = config.rpc?.providers ? Object.keys(config.rpc.providers) : [];
    const builtinProviders = ['alchemy', 'infura', 'quicknode', 'ankr', 'tatum', 'helius',
      'trongrid', 'publicnode', 'cloudflare', 'solana-public', 'xlayer-public', 'xlayer-okx'];
    this.knownProviders = new Set([...configProviders, ...builtinProviders])
  ...
  private validateRpcConfig(rpcConfig: any, result: ValidationResult): void {
    // Validate API keys
    ...
    if (rpcConfig.chains) {
      for (const [chain, chainConfig] of Object.entries(rpcConfig.chains)) {
        this.validateChainConfig(chain, chainConfig as any, result)
      }
    }
  }

  private validateChainConfig(chain: string, chainConfig: any, result: ValidationResult): void {
    // Validate chain name
  ...
  private validateMonitorConfig(monitorConfig: any, result: ValidationResult): void {
  ...
  private validateRequestedChains(requestedChains: string[], rpcConfig: any, result: ValidationResult): void {
    const availableProviders = Object.keys(rpcConfig.apiKeys || {})
    // Also include providers that are defined in the config (public providers don't need API keys)
    const definedProviders = Object.keys(rpcConfig.providers || {})
    // Built-in public providers (authType: 'none') are always available without API keys
    const publicProviders = ['publicnode', 'cloudflare', 'solana-public', 'xlayer-public', 'xlayer-okx']
    const allAvailableProviders = new Set([...availableProviders, ...definedProviders, ...publicProviders])
    ...
    for (const chain of requestedChains) {
      ...
      const hasProviders = chainProviders.some(provider => allAvailableProviders.has(provider))
  ...
  private provideSuggestions(config: LoadedConfig, result: ValidationResult): void {
  ...
  private suggestProvider(provider: string): string | null {
    const providers = Array.from(this.knownProviders)
    return this.findClosestMatch(provider, providers)
  }

  private suggestChain(chain: string): string | null {
  ...
  private getProvidersForChain(chain: string): string[] {
    // Map chains to their typical providers
    const chainProviders: Record<string, string[]> = {
      ...
      'tron-nile': ['trongrid'],
      'solana-mainnet': ['helius', 'solana-public'],
      'solana-devnet': ['helius', 'solana-public'],
      'arbitrum-mainnet': ['alchemy', 'ankr', 'publicnode'],
      'arbitrum-sepolia': ['alchemy', 'publicnode'],
      'xlayer-mainnet': ['xlayer-public', 'xlayer-okx'],
      'xlayer-testnet': ['xlayer-public', 'xlayer-okx']
    }
 ...
 */
export function validateConfig(config: LoadedConfig, requestedChains?: string[]): ValidationResult {
