...
import { BUILTIN_PROVIDERS } from './provider-templates.js'
import {
  ConfigLoader,
  RPCProviderTemplate,
  RPCGlobalConfig,
...
export class RPCConfigLoader implements ConfigLoader {
  private config!: ConfigFormat
  private availableProviders: Map<string, RPCProviderTemplate> = new Map()
  private readonly logger = createLogger(LogCategory.RPC)

  constructor(
    private configPath?: string,
    private rpcKeys: RPCProviderKeys = {},
    private rpcConfig?: ConfigFormat,
    private disablePublicProviders: boolean = false
  ) {}

  async loadProviderTemplates(): Promise<RPCProviderTemplate[]> {
    await this.loadConfigFile()
    return Array.from(this.availableProviders.values())
  }

  async loadGlobalConfig(): Promise<RPCGlobalConfig> {
    await this.loadConfigFile()
    
    const chains: Record<string, RPCChainConfig> = {}

    for (const [chainName, chainConfig] of Object.entries(this.config.chains)) {
      const availableEndpoints: RPCEndpointConfig[] = []
  ...
  private parseConfig(content: string): ConfigFormat {
    if (!this.configPath) {
  ...
  private async buildProviderTemplates(): Promise<void> {
    this.availableProviders.clear()

    // Priority: code providers > YAML customProviders > built-in providers
    // Always start with built-in providers as base, then overlay custom configs
    const providers: Record<string, any> = {}

    // 1. Load built-in providers as base defaults
    Object.assign(providers, BUILTIN_PROVIDERS)

    // 2. Load YAML customProviders (override built-in if same name)
    if (this.config.customProviders) {
      Object.assign(providers, this.config.customProviders)
    }

    // 3. Load code providers (highest priority, override everything)
    ...
    for (const [providerName, providerConfig] of Object.entries(providers)) {
      ...
      const provider: RPCProviderTemplate = {
        name: providerName,
